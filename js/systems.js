...
(function (K) {
  ...
  P.guides = id => {
    ...
    return `<h2>Guides</h2>${K.profileBar()}<div class="sub-tabs">${G.map(x => `<a href="#guides/${x.id}" class="${x.id === g.id ? 'on' : ''}">${esc(x.title)}</a>`).join('')}</div>${g.intro ? `<p class="small">${rich(g.intro)}</p>` : ''}${g.plain ? '' : `<p><b>Main stat:</b> ${['STR', 'AGI', 'INT'].map(s => `<label style="margin-right:10px"><input type="radio" name="stat" value="${s}" ${s === stat ? 'checked' : ''}> ${s}</label>`).join('')} <span class="small">items switch to your stat's version</span> &nbsp; <span class="small">☑ tick a step when done - everything above it is ticked too, unticking clears everything below. Early-game ticks also mark the same items in the gear guide (this browser)</span></p>`}${secs.length > 1 ? subtabs('guides/' + g.id, 'gsec', secs.map(s => [s.h, s.h]), cur) : ''}<div class="card"><h3>${esc(g.title)}${secs.length > 1 ? ' · ' + esc(cur) : ''}</h3>${secs[si].table ? `<div class="tbl"><table><tr>${secs[si].table.columns.map(c => `<th>${esc(c)}</th>`).join('')}</tr>${secs[si].table.rows.map(r => `<tr>${r.map((c, ci) => `<td class="${ci === 0 ? 'num' : ci === 1 ? '' : 'small'}">${ci === 1 ? `<b>${esc(c)}</b>` : esc(c)}</td>`).join('')}</tr>`).join('')}</table></div>` : stepsHtml(secs[si].steps, g.id + ':' + si)}</div>`;
  };
  const CHANGES = [
    ['22 Sep 2026', [
      ['Planner', [
