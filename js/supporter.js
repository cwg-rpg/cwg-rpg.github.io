...
(function (K) {
  ...
  const overview = () => {
    ...
    return `<p class="small">Bought through a Discord ticket, rewards are account-wide.</p>
    <div class="grid">
      <div class="card"><h3>Hero unlock - $20</h3><p>Unlocks one supporter hero for your account. Hero swap $25; swap + unlock $35.</p><p class="small">${[...new Set(heroes.flatMap(h => (h.items || []).map(o => { const u = (W.heroes || []).find(x => x.id === o.id); return u ? u.lineage : ''; }).filter(Boolean)))].map(l => link('hero', l, l)).join(' · ')}</p><p>${link('heroes', '', 'Heroes')}</p></div>
      <div class="card"><h3>Wings - $10 each</h3><p>+3% attack speed per wing owned, all 14 add another +20%.</p><p>${link('supporter?tab=wings', '', 'Wings')}</p></div>
  ...
  const tiers = () => `<p>Permanent EXP / Gold / Drop bonus. Each tier is a separate purchase.</p><div class="tbl"><table><tr><th>Tier</th><th class="num">EXP / Gold / Drop</th><th class="num">Price</th></tr>${TIERS.map(([n, p, d]) => `<tr><td>Tier ${roman(n)}</td><td class="num">+${p}%</td><td class="num">$${d}</td></tr>`).join('')}</table></div><p class="small">There is no maximum tier: after Tier XVI ($220) every further tier adds +35% EXP / Gold / Currency for +$30. The bonus is capped at +1000%.</p>`;
  const wings = () => `<div class="card hi"><h3>Wing bonuses (permanent)</h3><ul style="margin:0"><li>Per owned wing: +3% attack speed.</li><li>Own all 14 wings: another +20% attack speed.</li></ul></div><p class="small">The shown wing is cosmetic: type w1 to w14 to pick one, w0 for automatic, -wingoff to hide it. A hidden wing keeps its bonus.</p>`;
  const auras = () => `<div class="card hi"><h3>Aura bonuses (permanent)</h3><ul style="margin:0"><li>Per owned aura: +3% damage (Strength + Agility + Intelligence x 3% on every attack).</li><li>Own all 14 auras: another +28% damage.</li></ul></div><p class="small">The shown aura is cosmetic: a1 to a14 picks one, a0 is automatic, -auraoff hides it, -auraon shows it. A hidden aura keeps its bonus.</p>`;
  ...
  P.pets = id => {
    if (id) { const p = PE.pets.find(x => x.code === id); if (!p) return '<p>Unknown pet.</p>';
      return `<h2>${esc(p.name)}</h2><p class="small">${link('pets', '', 'All pets')}</p><div class="card"><div class="kv">${p.passive ? `<b>Bonus to you</b><span>${esc(p.passive.text)}</span><b>Value</b><span>${esc(p.passive.base)}${p.passive.evolution_stone && !/^no/i.test(p.passive.evolution_stone) ? ` · with Evolution Stone ${esc(p.passive.evolution_stone)}` : ''}${p.passive.full_collection ? ` · full collection ${esc(p.passive.full_collection)}` : ''}${p.passive.both ? ` · both ${esc(p.passive.both)}` : ''}</span>` : `<b>Bonus to you</b><span>none, only the aura below</span>`}<b>Aura / effect of the pet</b><span>${ul((p.summoned || []).filter(s => !/^Magic immunity/.test(s)))}</span>${p.evolved ? `<b>Evolved form</b><span>${mlink(p.evolved.id)} (Evolution Stone in the pet's inventory)</span>` : ''}${p.obtained_item ? `<b>How to get</b><span>carry ${ilink(p.obtained_item.id)}</span>` : ''}</div></div>`; }
    return `<h2>Pets</h2>
    <div class="card hi"><ul style="margin:0"><li>Pets 1-12 are supporter rewards ($15 each); Lumipaca comes from the Alpaca bundle item.</li><li>Pet slot 1 is for sale. Slot 2 is not for sale yet.</li><li>Only the pet you have out gives its bonus, owning several does not stack them.</li><li>If you own Kain, equipping another pet gives a weaker lifesteal.</li></ul></div>
    <div class="tbl"><table><tr><th>Pet</th><th>Bonus to you</th><th>Aura / effect of the pet itself</th></tr>${PE.pets.map(p => `<tr><td>${link('pets', p.code, p.name.split(',')[0].replace(' - Hero Form', ''))}</td><td class="small">${p.passive ? `${esc(first(p.passive.text).replace(/:.*$/, ''))}: <b>${esc(p.passive.base)}</b>` : '-'}</td><td class="small">${aura(p).map(esc).join('<br>') || '-'}</td></tr>`).join('')}</table></div>
    <h3>Companions</h3><p class="small">Bag items: put one in a storage bag and the bag turns into the companion. Anya comes free with every hero and does the auto-pickup (-autopickup).</p><div class="tbl"><table><tr><th>Companion</th><th>Bonus</th></tr>${CO.items.map(c => `<tr><td>${ilink(c.item.id)}</td><td class="small">${esc((s => /^No (aura|stats)/.test(s) ? (/Anya/.test(c.item.name) ? 'Auto-pickup helper' : 'Cosmetic') : s.charAt(0).toUpperCase() + s.slice(1).replace(/[;,]$/, '.'))(String(c.effect).replace(/^Ownership bonus \([^)]*\): /, '').split(/(?<=[.;])\s/)[0]))}</td></tr>`).join('')}</table></div>
