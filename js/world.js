...
(function (K) {
  ...
  P.home = () => {
    ...
    <div class="grid">
      ...
      ${card('monsters', 'Monsters', `${Object.keys(M).length} monsters and bosses by zone - HP, gold, EXP and every drop`)}
      ...
      ${card('calc', 'Calculators', 'Drop chance and enhancement cost')}
  ...
  P.zone = id => {
    ...
    <div class="card"><div class="kv"><b>Entry</b><span>${esc(z.entry)}</span><b>How</b><span>${esc(z.how)}</span>${farmList.length ? `<b>Worth farming</b><span>${farmList.map(([k, v]) => ilink(k) + ' <span class="small">' + pct(v.c) + ' · ' + mlink(v.m) + '</span>').join('<br>')}</span>` : ''}${(z.notes || []).length ? `<b>Notes</b><span>${gen(z.notes)}</span>` : ''}</div></div>
    ${mtable(z.normal_monsters, 'Normal monsters')}${mtable(z.field_bosses, 'Field bosses')}
    ${(z.raids || []).map(rd => `<h4>Raid · ${esc(rd.name)}</h4><p class="small">${esc(rd.entry || '')}</p>${mtable(inZone(id, 'raid boss', rd.name).length ? inZone(id, 'raid boss', rd.name) : (rd.bosses || []).map(b => ({ id: b.id })), 'Bosses')}`).join('')}
    ${(z.tickets || []).map(td => `<h4>Ticket dungeon · ${esc(td.name)}</h4><p class="small">Ticket: ${td.ticket && I[td.ticket.id] ? ilink(td.ticket.id) : esc(td.ticket && td.ticket.name || '')}${td.ticket_source ? ' - ' + esc(td.ticket_source) : ''}<br>${esc(td.how || '')}</p>${mtable(inZone(id, 'ticket dungeon monster', td.name), 'Monsters')}${mtable(inZone(id, 'ticket boss', td.name).length ? inZone(id, 'ticket boss', td.name) : (td.bosses || []).map(b => ({ id: b.id })), 'Bosses')}`).join('')}
  ...
  const zoneMons = zid => { const seen = new Set(); const out = []; for (const m of Object.values(M)) for (const z of m.zones) { if (z.zone !== zid || seen.has(m.id)) continue; seen.add(m.id); out.push({ m, z }); } const rank = { normal: 0, 'field boss': 1, 'ticket dungeon monster': 2, 'ticket boss': 3, 'raid boss': 4 }; return out.sort((a, b) => (rank[a.z.kind] || 5) - (rank[b.z.kind] || 5) || (parseInt(a.z.level) || 0) - (parseInt(b.z.level) || 0)); };
  ...
  P.monsters = (_, f) => {
    ...
    const head = `<h2>Monsters</h2><p class="small">Drop chances are base values, drops land on the ground for anyone. Gold and EXP are per kill (gold above 1,000,000 becomes lumber).</p>${subtabs('monsters', 'view', [['zone', 'By zone'], ['all', 'One table']], view)}`;
    ...
    return `${head}${subtabs('monsters', 'kind', [['', 'All'], ...kinds.map(x => [x, x])], k)}${filterBox('filter by name', f.q)}<p class="small">${rows.length} monsters in zone order. Click a column header to sort.</p>
    <div class="tbl"><table class="sortable"><tr><th>Monster</th><th>Kind</th><th class="num">Lv</th><th>Where</th><th class="num">Gold</th><th class="num">EXP</th><th>Drops</th></tr>${rows.slice(0, 400).map(m => `<tr><td>${mlink(m.id)}</td><td class="small">${esc(m.kind)}</td><td class="num">${lv(m) != null ? lv(m) : ''}</td><td class="small">${[...new Set(m.zones.map(x => x.zone))].map(zlink).join('; ')}</td><td class="num">${fmt(m.gold)}</td><td class="num">${fmt(m.exp)}</td><td class="small">${[...m.drops].sort((a, b) => b.chance - a.chance).slice(0, 4).map(d => ilink(d.item) + ' ' + pct(d.chance)).join(' · ')}${m.drops.length > 4 ? ` · +${m.drops.length - 4}` : ''}</td></tr>`).join('')}</table></div>`;
  };
  P.monster = id => {
